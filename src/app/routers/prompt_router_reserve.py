...
class PromptHandler:
    ...
    def run(self, input_text: str, dataframes_list: List[pd.DataFrame]) -> Dict[str, Any]:
        agent = Agent(dataframes_list, config={"llm": self.llm, "verbose": True, "enable_cache": False, "max_retries": 10,  "custom_whitelisted_dependencies": ["scikit-learn"]})
        rephrased_query = agent.rephrase_query(input_text)
        response_content = agent.chat(rephrased_query)
...
class GenerateInsightRecommendationOptimization:
    ...
    def answer_questions(self, question_list: list[str, Any], response_content: Dict[str, Any]) -> Dict[str, Any]:
        answer_dict = dict()
        output = convert_table_to_dataframe(response_content["table"])
        agent = Agent(output, config={"llm": self.llm, "verbose": True, "enable_cache": False, "max_retries": 10,  "custom_whitelisted_dependencies": ["scikit-learn"]})
...
class PromptFacade:
    ...
    async def handle_prompt(self, input_text: str, board_id: str, user_name:str, use_cache: bool) -> Dict[str, Any]:
        try:
            ...
            if existing_response and use_cache:
                if isinstance(existing_response, dict):  # If it’s a dictionary, access normally
                    return existing_response.get("response", {})
                elif hasattr(existing_response, "response"):  # If it's an object, use attribute
                    return existing_response.response
